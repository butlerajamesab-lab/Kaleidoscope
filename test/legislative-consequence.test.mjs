...
test('binds legislation through Docket Room and Civic Genome without collapsing ownership', () => {
  ...
  assert.equal(rosetta.extraction_run_id, '26');
...
test('preserves the pre-run Rosetta gap and the post-run assembly transition explicitly', () => {
  const transition = fixture.legislation_platform_bindings.run_transition;
  assert.equal(transition.pre_run_observation.rosetta_source_binding_count, 0);
  assert.equal(transition.pre_run_observation.completed_assembly_count, 0);
  assert.equal(transition.pre_run_observation.trait_count, 0);
  assert.equal(transition.post_run_observation.rosetta_source_binding_count, 1);
  assert.equal(transition.post_run_observation.completed_assembly_count, 1);
  assert.equal(transition.post_run_observation.trait_count, 7);
