...
**Status:** Staging foundation and deterministic scaffold v0.1.4. Typed diff, canonical hashing, complete source custody, authenticated Civic Genome snapshot validation, the Project 2025 Title VII executable test fixture and read-only inspection shell, and the bounded EEOC Legislative Consequence Stage 1/2 specimen are source-controlled. The append-only Kaleidoscope Supabase projection substrate is applied and empty. A governed runtime persistence adapter, production projection capability, and a live accepted Civic Genome binding are not yet claimed.
...
A separate bounded EEOC workforce-demographics rollback specimen now exercises the Legislative Consequence Stage 1/2 contracts. It is a structural/consequence contract specimen, not a completed Kaleidoscope projection.
...
- Render: `srv-d9ob6re417fc73euovag`—last source-controlled deployment receipt is v0.1.4; current post-Project-2025 and consequence-engine source changes require deliberate runtime verification before being represented as live
