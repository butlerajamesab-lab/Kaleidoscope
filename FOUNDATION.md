...
Kaleidoscope now has a source-controlled inspection workspace with a truthful browser/machine boundary:

- browser `GET /` with `Accept: text/html` serves the workspace;
- `GET /app` serves the explicit workspace path;
- non-HTML `GET /` remains machine-readable JSON;
- `GET /v1/platform/read-model` serves the deterministic platform state used by the workspace;
- `GET /project2025/title-vii` remains the detailed first scenario inspection surface.

The workspace exposes Overview, Scenarios, Lenses, Sources, Receipts, and System views. It reads source-controlled receipts and fixtures, fails closed on identity/state mismatches, and does not expose an ungoverned run or persistence control.
...
Current v0.1.4 source control proves complete source custody, canonicalization, hashing, typed diff behavior, Civic Genome source-contract validation and tamper rejection, authenticated non-persistent Civic Genome delivery validation, an executable bounded Project 2025 test fixture, its detailed inspection surface, the full platform inspection workspace, and the bounded Legislative Consequence Stage 1/2 specimen. The append-only Supabase projection substrate and its foreign-key indexes are live and empty. Production projection execution, Legislative Consequence stages 3 through 6, a governed runtime persistence adapter, an accepted Civic Genome binding, persisted replay, full peer-platform bridge proofs, and production-grade user operation remain unproven.
